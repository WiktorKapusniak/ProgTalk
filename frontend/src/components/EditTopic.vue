...
onMounted(async () => {
  try {
    ...
    form.topic = res.data.title;
    form.description = res.data.description;
  } catch (err: any) {
    toast.error(err.response?.data?.message || "Błąd ładowania tematu");
  }
});

const handleEditTopic = async () => {
  try {
    const response = await axios.patch(`/api/topics/${topicId}`, {
      title: form.topic,
    ...
    toast.error(err.response?.data?.message || "Błąd edycji tematu");
  }
};
</script>
